  ...
  try {
    ...
    if (verifiedUser) {
      res
        .status(409)
        .json(
          "User already exist with provided username!!"
        );
      return;
    }
    ...
    if (verifiedUser) {
      res
        .status(409)
        .json("User already exist with provided Email!!");
      return;
    }
    ...
    // let findUser = await UnverifiedUser.findOne({
    //   name: name,
    // });
    // if (findUser) {
    //   jwt.verify(
    ...
    //     async (err, data) => {
    //       if (err) {
    //         await UnverifiedUser.findOneAndDelete({
    //           name: name,
    //         });
    //       } else {
    //         res
    ...
    // findUser = await UnverifiedUser.findOne({
    //   email: email,
    // });
    // if (findUser) {
    //   jwt.verify(
    ...
    //           .status(409)
    //           .json(
    //             "A verification mail has already been sent!!"
    //           );
    //         return;
    //       }
    //     }
    ...
    //   res
    //     .status(201)
    //     .json("Verification mail sent successfully!");
    // } catch (err) {
    //   res
    //     .status(500)
    //     .json("Internal Error while sending mail!");
    //   return;
    // }
  } catch (err) {
...
exports.verifyAccount = catchAsync(async (req, res) => {
  ...
  try {
    ...
    res.redirect(
      `${process.env.FRONTEND_BASE_URL}/response?status=503`
    );
  }
});
...
exports.forgotPassword = catchAsync(
  async (req, res, next) => {
    ...
    if (!user) {
      res
        .status(404)
        .json("User does not exist in the system!");
      return;
    }
...
exports.addToStarred = catchAsync(async (req, res) => {
  ...
  try {
    ...
    if (likingUser.favourites.indexOf(postData._id) === -1)
      ...
      likingUser.favourites.splice(
        likingUser.favourites.indexOf(postData._id),
        1
      );
