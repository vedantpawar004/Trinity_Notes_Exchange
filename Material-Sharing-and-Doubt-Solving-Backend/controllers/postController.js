...
exports.createPost = catchAsync(async (req, res) => {
  const { title, description, tags, media, user } =
    req.body;
  try {
    console.log("new post ceated", user.data._id);
    ...
    // console.log("new post ceated", newPost);
    // var use = User.findOne(user.data._id);
    // use.materialCount += 1;
    // await use.save();

    res.status(201).json(newPost);
  } catch (err) {
    console.log("from post created", err);
    res
      .status(500)
      .json("Couldn't create post!! Please try again!");
  }
});
...
exports.fetchAll = catchAsync(async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(
        "Couldn't find posts!! Please refresh and try again!!"
      );
  }
});
  ...
  try {
    ...
    if (type === "up") {
      ...
      if (post.downVotes.indexOf(user._id) !== -1) {
        post.downVotes.splice(
          post.downVotes.indexOf(user._id),
          1
        );
        owner.reputation += 1;
      }
    ...
    res
      .status(200)
      .json({ postData: post, ownerInfo: owner });
  } catch (err) {
    res
      .status(500)
      .json(
